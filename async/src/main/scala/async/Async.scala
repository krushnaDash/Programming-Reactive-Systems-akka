...
object Async extends AsyncInterface {
  ...
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = {
    eventuallyX.transform(s => s % 2 == 0, cause => cause)
  }
  ...
  def sequenceComputations[A, B](
   ...
   // makeAsyncComputation1() zip  makeAsyncComputation2() // this run at the same time so we need flatmap
    makeAsyncComputation1().flatMap( c1=> makeAsyncComputation2().map(c2 => (c1,c2)) )
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    var attempt = 1
    var result=makeAsyncComputation()
    while (attempt < maxAttempts) {
      result = result.recoverWith{case e:Exception => makeAsyncComputation()}
      attempt=attempt+1;
    }
    result
  }
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = {
    ...
    callbackBasedApi.computeIntAsync(
      value => {
        if(value.isSuccess)
          p.trySuccess(value.get)
        else
          p.tryFailure(value.failed.get)
      }
    )
    () => p.future // this mean new FutureBasedApi { override def computeIntAsync(): Future[Int] = p.future }
  }
